 ...
 * Author: fdilf
 * 
 * Created on September 20, 2022, 1:38 PM
 */
...
uint8_t prevInput = 0xFF;   //Initialize with an impossible value
uint8_t input;
...
int main(void) {
    ...
    while(1)
    {
        //First read input
        input = read_input();   
        // Then compare the previous input.

        //If input didn't change, then can keep the output the same as well
        // Meaning no need to do anything
        if (prevInput != input)
        {
            display_UCID_digit(input);
            //Before continuing the loop, update prevInput with new/current one
            prevInput = input;
        }
    }
    return 0;
